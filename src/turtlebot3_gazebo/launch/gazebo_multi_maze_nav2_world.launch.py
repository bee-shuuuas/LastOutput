...
def generate_launch_description():
    ...
    robots = [
        {'name': 'tb1', 'x_pose': '-1.5', 'y_pose': '-3.5', 'z_pose': 0.01},
        {'name': 'tb2', 'x_pose': '-8.5', 'y_pose': '0.5', 'z_pose': 0.01},

        # {'name': 'tb2', 'x_pose': '-10.5', 'y_pose': '10.5', 'z_pose': 0.01},

        # {'name': 'tb3', 'x_pose': '1.5', 'y_pose': '-0.5', 'z_pose': 0.01},
        # {'name': 'tb4', 'x_pose': '1.5', 'y_pose': '0.5', 'z_pose': 0.01},
        {'name': 'tb3', 'x_pose': '5.5', 'y_pose': '0.5', 'z_pose': 0.01},
        # ...
        # ...
        ]
    ...
    urdf = os.path.join(
        turtlebot3_gazebo, 'urdf', 'turtlebot3_' + TURTLEBOT3_MODEL + '.urdf'
    )

    world = os.path.join(
        get_package_share_directory('turtlebot3_gazebo'),
        'worlds', 'Maze1_robot_world_obstacles.world')
    ...
    map_server=Node(package='nav2_map_server',
        ...
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': '/home/biswash/up_work/robot_ws/src/multi_robot_navigation/MAPs/Maze1.yaml'},],
    ...
    for robot in robots:
        ...
        spawn_turtlebot3_burger = Node(
            ...
            arguments=[
                '-file', os.path.join(turtlebot3_gazebo,'models', 'turtlebot3_' + TURTLEBOT3_MODEL, 'model.sdf'),
                '-entity', robot['name'],
    ...
    for robot in robots:
        ...
        #             output='screen',
        #             remappings=[
        #                 # (f'cmd_vel', f'{namespace}/cmd_vel/dd'),  # Remap /tbX/cmd_vel to /tbX/cmd_vel/dd
        #                 (f'cmd_vel', f'{namespace}/cmd_vel'),  # Remap /tbX/cmd_vel to /tbX/cmd_vel
        #                 (f'scan', f'{namespace}/scan'),          # Ensure scan topic is namespaced
        #             ],
        #             parameters=[{'use_sim_time': True}],
        #             condition=IfCondition(enable_drive)  # Still conditional, but enable_drive is now 'true'
        #         )
